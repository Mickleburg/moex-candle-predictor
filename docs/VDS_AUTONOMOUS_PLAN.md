...
Малы и принадлежат ML; финишируемы здесь. Порядок = ценность:
1. **MOEX holiday-aware счётчик торговых дней.** Заменить `np.busday_count` в
   ...
   на торговый календарь: для дат внутри панели — её собственный `DatetimeIndex` (ground truth, уже
   так для истории); для форвардных — `np.busdaycalendar` с RU-праздниками. **Correctness:** record-даты
   кластеризуются май-июль (праздники 1/9 мая, 12 июня). Приёмка: вход/выход не дрейфует через праздник.
2. **Свежий ценовой панель до сегодня + сверка live inverse-vol.** Докачать свечи по сегодня, чтобы
   live-сайзинг (as_of>панель) считал истинные трейлинг-vol, а не клампился к последней. Сверка:
   `target_positions` на свежей панели воспроизводит holdings/размеры монитора.
3. **Realized-P&L атрибуция для shadow-гейта.** `h9_shadow_pnl.py`: по каждому закрытому событию —
   реализованный market-adjusted run-up (close входа → close выхода, минус хедж), накопление vs
   историческое +0.84%/событие (high-yield +1.45%) + placebo-полоса. **Это измеримый гейт для снятия
   `is_production=false`.**
4. **Сверка якоря (record vs ex date).** Фид несёт и `record_date`, и `ex_date`; слив якорится на
   `record_date` (как `dividends.csv` из ISS — на чём валидировался research; T+1: ex = record−1).
   Подтвердить, что `record_date` фида = тот же объект, что в research; иначе поправить якорь. Дешёвая
   сверка, снимает интеграционную неоднозначность.
5. *(опц.)* **Live serving-CLI + emit контракта.** `predict_dividend_sleeve.py` пишет `sleeve_signal`
   JSON в известный путь, чтобы `agent/` потреблял ML без импорта внутренностей.
...
1. ML-доводка 1-4 закрыты (holiday-календарь + realized-P&L гейт построен). 
